...
use TesseractOcr\Ccstruct\Publictypes\PageIteratorLevel;
use TesseractOcr\Ccstruct\Publictypes\PageSegMode;

require_once 'TesseractOcr/Api/OcrEngineMode.php';
require_once 'TesseractOcr/Ccmain/Tesseract.php';

class TessBaseAPI {

    // Minimum sensible image size to be worth running tesseract.
    const kMinRectSize = 10;
    // Character returned when Tesseract couldn't recognize as anything.
    const kTesseractReject = '~';
    ...
    const kInputFile = "noname.tif";
    // Temp file used for storing current parameters before applying retry values.
    const kOldVarsFile = "failed_vars.txt";
    // Max string length of an int.
    const kMaxIntSize = 22;

    /**
     * @var TesseractOcr\Ccmain\Tesseract
     */
    ...
    */
    ...
    public function GetStringVariable(/*const char **/$name) {
        $p = ParamUtils::FindParam/*<StringParam>*/(
        $name, $this->GlobalParams()->string_params, $this->tesseract_->params()->string_params);
    ...
    public function GetDoubleVariable(/*const char **/$name, /*double **/$value) {
        ...
        $name, $this->GlobalParams()->double_params, $this->tesseract_->params()->double_params);
    ...
    public function GetVariableAsString(/*const char **/$name, STRING $val) {
        return ParamUtils::GetParamAsString($name, $this->tesseract_->params(), $val);
    }
    // Print Tesseract parameters to the given file.
    public function PrintVariables(/*FILE **/$fp) {
        ParamUtils::PrintParams($fp, $this->tesseract_->params());
    ...
    /*bool*/ $set_only_init_params) {
        ...
        if ($this->tesseract_ != NULL &&
        ($this->datapath_ == NULL || $this->language_ == NULL ||
        $this->datapath_ != $datapath || $this->last_oem_requested_ != $oem ||
        ($this->language_ != $language && $this->tesseract_->lang != $language))) {
            $this->tesseract_->end_tesseract();
            $this->tesseract_ = null;
        }

        $reset_classifier = true;
        if ($this->tesseract_ === NULL) {
            $this->reset_classifier = false;
            $this->tesseract_ = new Tesseract;
            if ($this->tesseract_->init_tesseract(
            $datapath, $this->output_file_ !== NULL ? $this->output_file_ : NULL,
        ...
        if ($this->datapath_ == NULL) {
            $this->datapath_ = new STRING($datapath);
        } else {
            $this->datapath_ = $datapath;
        }
        if ($this->language_ == NULL){
            $this->language_ = new STRING($language);
        } else {
            $this->language_ = $language;
            $this->last_oem_requested_ = $oem;
        }

        // For same language and datapath, just reset the adaptive classifier.
        if ($reset_classifier) {
            $this->tesseract_->ResetAdaptiveClassifier();
        }
    ...
    // @return int
    //public function Init(/*const char**/ $datapath,
    ///*const char**/ $language,
    //OcrEngineMode $oem) {
    ...
    public function InitLangMod(/*const char**/ $datapath, /*const char**/ $language) {
        ...
        return $this->tesseract_->init_tesseract_lm($datapath, null, $language);
    }
    ...
    public function InitForAnalysePage() {
        if ($this->tesseract_ === NULL) {
            $this->tesseract_ = new Tesseract;
            $this->tesseract_->InitAdaptiveClassifier(false);
        }
    }
    ...
    public function ReadConfigFile(/*const char**/ $filename, /*bool*/ $init_only) {
        $this->tesseract_->read_config_file($filename, $init_only);
    }
    ...
    public function SetPageSegMode(PageSegMode $mode) {
        ...
        $this->tesseract_->tessedit_pageseg_mode = $mode;
    }
    ...
    public function GetPageSegMode() {
        ...
        throw new \Exception("Analisar");
        //return static_cast<PageSegMode>(static_cast<int>($this->tesseract_->tessedit_pageseg_mode));
    }
    ...
    /*int*/ $left, /*int*/ $top, /*int*/ $width, /*int*/ $height) {
        if ($this->tesseract_ == NULL || $width < self::kMinRectSize || $height < self::kMinRectSize) {
        ...
        $bits_per_pixel = $bytes_per_pixel == 0 ? 1 : $bytes_per_pixel * 8;
        $this->SetImage($imagedata, $bytes_per_line * 8 / $bits_per_pixel, $height + $top,
    ...
    public function ClearAdaptiveClassifier() {
        ...
        $this->tesseract_->ResetAdaptiveClassifier();
        $this->tesseract_->getDict()->ResetDocumentDictionary();
    }
    ...
    public function SetImage(
    	...
    	/*int*/ $bytes_per_line = null) {

    	if ($width === null
    	    && $height === null
    	    && $bytes_per_pixel === null
    	    && $bytes_per_line === null) {
            if ($this->InternalSetImage()) {
                $this->thresholder_->SetImage($pix);
            }
            return null;
    	}

        if ($this->InternalSetImage()) {
            $this->thresholder_->SetImage($imagedata, $width, $height,
                $bytes_per_pixel, $bytes_per_line);
        }
    }
    ...
    public function SetRectangle(/*int*/ $left, /*int*/ $top, /*int*/ $width, /*int*/ $height) {
        if ($this->thresholder_ == NULL) {
            return ;
        }
        $this->thresholder_->SetRectangle($left, $top, $width, $height);
        $this->ClearResults();
    }
    ...
    public function GetThresholdedImage() {
        ...
        if ($this->tesseract_->pix_binary() == NULL) {
            $this->Threshold($this->tesseract_->mutable_pix_binary());
        }
        return $this->pixClone($this->tesseract_->pix_binary());
    }
    ...
    public function GetRegions(Pixa $pixa) {
        return $this->GetComponentImages(PageIteratorLevel::RIL_BLOCK(), $pixa, null);
    }
    ...
    public function GetTextlines(Pixa $pixa, /*int***/ $blockids) {
        return $this->GetComponentImages(PageIteratorLevel::RIL_TEXTLINE(), $pixa, $blockids);
    }
    ...
    // @return Boxa
    public function GetConnectedComponents(Pixa /*$cc*/ $pixa) {
        return GetComponentImages(PageIteratorLevel::RIL_SYMBOL(), $pixa, NULL);
    }
    ...
    public function GetWords(Pixa $pixa) {
        return $this->GetComponentImages(PageIteratorLevel::RIL_WORD(), $pixa, NULL);
    }
    ...
    Pixa $pixa, /*int***/ $blockids) {
        $page_it = $this->GetIterator();
        if ($page_it == NULL) {
            $page_it = $this->AnalyseLayout();
        }
        if ($page_it == NULL) {
            return NULL;  // Failed.
        }
        ...
        } while ($this->page_it->Next($level));

        $boxa = $this->boxaCreate($component_count);
        if ($pixa != NULL) {
            $pixa = $this->pixaCreate(component_count);
        }
        if ($blockids != NULL) {
            $blockids = array();
        }
        ...
        do {
            if ($page_it->BoundingBox($level, &$left, &$top, &$right, &$bottom)) {
                $lbox = $this->boxCreate($left, $top, $right - $left, $bottom - $top);
                $this->boxaAddBox($boxa, $lbox, L_INSERT);
                if ($pixa != NULL) {
                    $pix = $page_it->GetBinaryImage($level);
                    $this->pixaAddPix($pixa, $pix, L_INSERT);
                    $this->pixaAddBox($pixa, $lbox, L_CLONE);
                }
                if ($blockids != NULL) {
                    $blockids[$component_index] = $blockid;
                    if ($page_it->IsAtFinalElement(PageIteratorLevel::RIL_BLOCK(), $level)) {
                        ++$blockid;
                    }
                }
                ++$component_index;
